    ...
    while (win.isOpen())  // Игровой цикл
    {
        ...
        while (win.pollEvent(event))  // Цикл игровых событий: нажатие клавишь, перемещение мышки и другие.
        {
            ...
            case Event::KeyPressed:
                //Управление игрой
                if (event.key.code == Keyboard::F1) GamePause = !GamePause;
                ...
                if ((event.key.code == Keyboard::Numpad5) || (event.key.code == Keyboard::S) || (event.key.code == Keyboard::PageDown))
                    if (full != 0) { moveRec.y = 0.3 * timePlayer; traffic = 2; minusfull = true;}
                if ((event.key.code == Keyboard::Numpad8) || (event.key.code == Keyboard::W) || (event.key.code == Keyboard::PageUp))
                    if (full != 0) { moveRec.y = -0.3 * timePlayer; traffic = 1; minusfull = true;  }
                if ((event.key.code == Keyboard::Numpad4) || (event.key.code == Keyboard::A) || (event.key.code == Keyboard::Home))
                    if (full != 0) { moveRec.x = -0.3 * timePlayer; minusfull = true;         }
                if ((event.key.code == Keyboard::Numpad6) || (event.key.code == Keyboard::D) || (event.key.code == Keyboard::End))
                    if (full != 0) { moveRec.x = 0.3 * timePlayer; minusfull = true;         }
                break;
